...
pub async fn vitalsource_rollback_groups(
    ...
    if let Ok(data) = fs::read_to_string(&config.file_paths.vitalsource_group_assignment_mapping) {
        if let Ok(existing_mapping) = serde_json::from_str::<GroupAssignmentMapping>(&data) {
            group_assignment_mapping = existing_mapping;
        }
    }

    for (bi_group_id, group_members) in group_assignment_mapping {
